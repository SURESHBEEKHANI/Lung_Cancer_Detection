...
from src.services.skin_service import run_skin_disease_inference

skin_router = APIRouter(prefix="/skin", tags=["Skin Disease"])


@skin_router.post("/predict")
async def predict_skin_disease(file: UploadFile = File(...)):
    ...
    # Run inference
    output = run_skin_disease_inference(image)

    # Convert output to BytesIO PNG
    buf = io.BytesIO()
