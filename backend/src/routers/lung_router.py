...
from src.services.lung_service import run_lung_cancer_inference

lung_router = APIRouter(prefix="/lung", tags=["Lung Cancer"])


@lung_router.post("/predict")
async def predict_lung_cancer(file: UploadFile = File(...)):
    ...
    output = run_lung_cancer_inference(image)
